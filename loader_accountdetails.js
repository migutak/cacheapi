  ...
  try {
    ...
    for (i = 0; i <= dataArray.length - 1; i++) {
      try {
        ...
        const response = await axios.post(soaurl + '/REST/Account/AccountDetails/Get/3.0', body);
        if (response.statusText = 'OK') {
          console.log(JSON.stringify(response.data));
          redis_client.set('accountdetails_' + body.CustomerDetailsInquiryRequest.CustomerDetailsInquiryRq.CustomerId, JSON.stringify(response.data));
        } else {
